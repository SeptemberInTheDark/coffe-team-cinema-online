...
@genres_router.post(
    path="/add_genre", summary="Добавить жанр", response_description="Добавленный жанр"
)
async def add_genre(
    ...
    try:
        ...
        if existing_genre:
            return JSONResponse(status_code=400, content={"error": "Такой жанр уже существует."})
...
@genres_router.delete(
    path="/delete_genre", summary="Удалить жанр", response_description="Удаленный жанр"
)
async def delete_movie(session: AsyncSession = Depends(get_db), name: str = Form(...)):
    try:
        deleted = await GenreCRUD.delete_genre(session, name=name)
        if deleted:
            logger.info("Жанр %s удален", name)
            return JSONResponse(
                status_code=200, content={"message": f"Жанр '{name}' успешно удален."}
            )
        else:
