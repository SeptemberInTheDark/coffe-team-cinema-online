...
router = APIRouter(prefix="/api/auth", tags=["Авторизация пользователя"])
...
async def auth_user(
    ...
    try:
        ...
        if decode_pass:
            ...
            return JSONResponse(
                content={"error": "Такого пароля не существует"},
                status_code=status.HTTP_401_UNAUTHORIZED,
            )
...
async def get_current_user(request: Request):
    ...
    if token is None:
        raise HTTPException(
            status_code=401, detail="Срок действия сессии истёк, пожалуйста, авторизуйтесь заново."
        )
